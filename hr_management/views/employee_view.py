...
def generate_focus_explanation(task):
    ...
    return " | ".join(reasons) if reasons else "This task needs attention based on sprint workload"
...
class EmployeeDashboardMetrics(APIView):
    ...
    def get(self, request):
        try:
            ...
            attendance_today = Attendance.objects.filter(employee=employee, date=today).exists()
            ...
            if active_sprint:
                tasks_qs = Task.objects.filter(sprint=active_sprint, assigned_to=request.user)
            ...
            ).exclude(status='DONE').order_by('due_date', '-priority', 'progress_percentage').first()
            ...
            if focus_task:
                ai_focus_task = {
                    ...
                    'due_date': focus_task.due_date.strftime('%Y-%m-%d') if focus_task.due_date else None,
                    'sprint_id': str(focus_task.sprint.id) if focus_task.sprint else None,
